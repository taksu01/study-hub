      ...
      <div className="p-6 sm:p-8">
                ...
                <div key={i}>
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all cursor-pointer border-2
                      ${miningStep === i ? 'border-gray-800 bg-gray-50 shadow-md' : 'border-gray-100 hover:border-gray-200'}`}
                  >
        ...
        {tab === 'difficulty' && (
          <div className="flex flex-col items-center gap-6 max-w-md mx-auto">
            <div className="w-full rounded-xl bg-blue-50 border border-blue-200 p-5 text-center">
              <p className="text-sm font-bold text-blue-800 mb-2">Difficulty Adjustment</p>
              <p className="text-xs text-blue-700">Every <strong>2,016 blocks</strong> (~2 weeks)</p>
            </div>
            <div className="grid grid-cols-2 gap-4 w-full">
              <div className="rounded-xl border-2 border-red-200 bg-red-50 p-4 text-center">
                <p className="text-2xl mb-1">🔥</p>
                <p className="text-xs font-bold text-red-700">Blocks found too FAST</p>
                <p className="text-xs text-red-600 mt-1">Target decreases</p>
                <p className="text-[10px] text-red-500 mt-0.5">(harder to mine)</p>
              </div>
              <div className="rounded-xl border-2 border-blue-200 bg-blue-50 p-4 text-center">
                <p className="text-2xl mb-1">🐢</p>
                <p className="text-xs font-bold text-blue-700">Blocks found too SLOW</p>
                <p className="text-xs text-blue-600 mt-1">Target increases</p>
                <p className="text-[10px] text-blue-500 mt-0.5">(easier to mine)</p>
              </div>
            </div>
            <div className="w-full rounded-xl bg-emerald-50 border border-emerald-200 p-4 text-center">
              <p className="text-xs font-bold text-emerald-700">Goal: maintain ~10 minute average block interval</p>
              <p className="text-[10px] text-emerald-600 mt-1">More hash power = more security, NOT faster blocks</p>
            </div>
          </div>
        )}
        ...
        {tab === 'halving' && (
          <div className="max-w-md mx-auto">
            <p className="text-xs text-gray-500 text-center mb-4">Block reward = Subsidy + Transaction fees. Subsidy halves every 210,000 blocks.</p>
            <div className="space-y-2">
              {halvings.map((h, i) => (
                <div key={i} className="flex items-center gap-3">
                  <span className="text-xs font-mono text-gray-400 w-12 text-right shrink-0">{h.year}</span>
                  ...
                  <span className="text-[10px] text-gray-400 w-32 shrink-0 truncate">{h.period}</span>
                </div>
              ))}
            </div>
            <div className="mt-4 rounded-xl bg-amber-50 border border-amber-200 p-3 text-center">
              <p className="text-xs text-amber-800">Total supply cap: <strong>21,000,000 BTC</strong></p>
              <p className="text-[10px] text-amber-600 mt-0.5">~99% mined by ~2035. Last satoshi by ~2140.</p>
            </div>
          </div>
        )}
      </div>
