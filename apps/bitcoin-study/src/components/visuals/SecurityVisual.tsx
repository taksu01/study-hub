    ...
    <div className="my-6 rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        ...
        <div className="flex gap-1">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer
              ${view === 'protocol' ? 'bg-gray-900 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'}`}
          >
            Protocol Threats
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer
              ${view === 'user' ? 'bg-gray-900 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'}`}
          >
            User Threats
          </button>
        </div>
      ...
      <div className="p-6 sm:p-8">
        ...
        <div className="flex items-center justify-center gap-4 mb-6">
          <div className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all ${view === 'protocol' ? 'bg-blue-100 border-2 border-blue-300' : 'bg-gray-50 border-2 border-transparent'}`}>
            <div className="w-2 h-2 rounded-full bg-blue-500" />
            <span className="text-xs font-medium text-blue-800">Protocol Level</span>
            <span className="text-[10px] text-blue-500">Very strong</span>
          </div>
          <div className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all ${view === 'user' ? 'bg-orange-100 border-2 border-orange-300' : 'bg-gray-50 border-2 border-transparent'}`}>
            <div className="w-2 h-2 rounded-full bg-orange-500" />
            <span className="text-xs font-medium text-orange-800">User Level</span>
            <span className="text-[10px] text-orange-500">Your responsibility</span>
          </div>
        </div>
              ...
              <div key={t.name}>
                <button
                  ...
                  className={`w-full rounded-xl border-2 p-4 text-left transition-all cursor-pointer
                    ${expandedProtocol === i ? 'border-blue-300 bg-blue-50 shadow-md' : 'border-gray-200 bg-white hover:border-gray-300'}`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400">{t.severity}</p>
                      </div>
                    </div>
                    <span className="text-xs text-gray-300">{expandedProtocol === i ? '▼' : '▶'}</span>
                  </div>
                </button>
                {expandedProtocol === i && (
                  <div className="mt-2 grid grid-cols-2 gap-2 px-2">
                    <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-3">
                      <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider mb-1.5">CAN do</p>
                      {t.can.map(c => (
                        <p key={c} className="text-xs text-emerald-700 flex items-start gap-1.5 mb-1">
                          <span className="text-emerald-400 mt-0.5 shrink-0">✓</span>{c}
                        </p>
                      ))}
                    </div>
                    <div className="rounded-lg bg-red-50 border border-red-200 p-3">
                      <p className="text-[10px] font-bold text-red-600 uppercase tracking-wider mb-1.5">CANNOT do</p>
                      {t.cannot.map(c => (
                        <p key={c} className="text-xs text-red-700 flex items-start gap-1.5 mb-1">
          ...
          <div className="space-y-3 max-w-lg mx-auto">
            {userThreats.map((t, i) => (
              <button
                ...
                className={`w-full rounded-xl border-2 p-4 text-left transition-all cursor-pointer
                  ${expandedUser === i ? 'border-orange-300 bg-orange-50 shadow-md' : 'border-gray-200 bg-white hover:border-gray-300'}`}
              >
                <div className="flex items-center gap-3">
                  <span className="text-xl">{t.icon}</span>
                  ...
                  <span className="text-xs text-gray-300">{expandedUser === i ? '▼' : '▶'}</span>
                </div>
                {expandedUser === i && (
                  <div className="mt-3 rounded-lg bg-emerald-50 border border-emerald-200 p-3">
                    <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider mb-1">Prevention</p>
                    <p className="text-xs text-emerald-700 leading-relaxed">{t.prevention}</p>
                  </div>
                )}
              </button>
            ))}
