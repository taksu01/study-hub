      ...
      <div className="p-6 sm:p-8">
        ...
        {tab === 'structure' && (
          <div className="flex flex-col items-center gap-4">
            <p className="text-xs text-gray-500 mb-2">Click any header field to learn more</p>
            <div className="w-full max-w-lg rounded-xl border-2 border-indigo-200 overflow-hidden">
                ...
                {headerFields.map((f, i) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 flex items-center justify-between gap-4 transition-all cursor-pointer
                      ${activeField === i ? 'bg-indigo-100' : 'hover:bg-indigo-50/50'}`}
                  >
                    <span className="text-sm font-medium text-gray-800">{f.label}</span>
                    <span className="text-xs font-mono text-gray-400 truncate max-w-[160px]">{f.value}</span>
                  </button>
                ))}
              </div>
            </div>
            {activeField !== null && (
              <div className="w-full max-w-lg rounded-xl bg-indigo-50 border border-indigo-200 p-4 transition-all">
                <p className="text-xs font-bold text-indigo-600 mb-1">{headerFields[activeField].label}</p>
                <p className="text-sm text-indigo-800 leading-relaxed">{headerFields[activeField].desc}</p>
              </div>
            )}
            ...
            <div className="w-full max-w-lg rounded-xl border-2 border-gray-200 overflow-hidden">
              <div className="bg-gray-50 px-4 py-2 border-b border-gray-200">
                <p className="text-xs font-bold text-gray-600 uppercase tracking-wider">Block Body — Transactions</p>
              </div>
              <div className="px-4 py-3 space-y-1.5">
                <div className="flex items-center gap-2 text-sm"><span className="w-2 h-2 rounded-full bg-amber-400" /><span className="font-medium text-gray-700">Coinbase TX</span><span className="text-xs text-gray-400">(miner reward)</span></div>
                {[1, 2, 3].map(n => (
                  <div key={n} className="flex items-center gap-2 text-sm"><span className="w-2 h-2 rounded-full bg-gray-300" /><span className="text-gray-600">Transaction {n}</span></div>
                ))}
                <div className="flex items-center gap-2 text-sm text-gray-400"><span className="w-2 h-2 rounded-full bg-gray-200" /><span>... TX N</span></div>
              </div>
            </div>
          </div>
        )}
        ...
        {tab === 'chain' && (
          <div>
            <p className="text-xs text-gray-500 mb-4 text-center">Click a block to "tamper" with it — see how it breaks the chain</p>
                  ...
                  <div key={b.id} className="flex items-center shrink-0">
                    {i > 0 && (
                      <div className="flex items-center mx-1">
                        <div className={`w-6 sm:w-10 h-0.5 transition-colors duration-300 ${isBroken ? 'bg-red-400' : 'bg-gray-300'}`} />
                        <svg className={`w-2 h-2 transition-colors duration-300 ${isBroken ? 'text-red-400' : 'text-gray-400'}`} viewBox="0 0 8 8" fill="currentColor"><path d="M0 0l8 4-8 4z"/></svg>
                      </div>
                    )}
                    <button
                      ...
                      className={`rounded-xl border-2 px-3 sm:px-4 py-3 text-center transition-all duration-300 min-w-[100px] cursor-pointer
                        ${isTampered ? 'border-red-400 bg-red-50 shadow-lg scale-105 ring-2 ring-red-200' :
                          isBroken ? 'border-red-300 bg-red-50/50' :
                          'border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm'}`}
                    >
                      <p className={`text-sm font-bold ${isTampered ? 'text-red-700' : isBroken ? 'text-red-600' : 'text-gray-800'}`}>{b.label}</p>
            ...
            {tamperedBlock !== null && (
              <div className="mt-4 rounded-xl bg-red-50 border border-red-200 p-4 max-w-lg mx-auto">
        ...
        {tab === 'merkle' && (
          <div className="flex flex-col items-center gap-2">
            <p className="text-xs text-gray-500 mb-2">Hover a transaction to see its hash path to the Merkle Root</p>

            {/* Root */}
            <div className={`rounded-lg border-2 px-5 py-2.5 text-center transition-all duration-200
              ${merkleHighlight ? 'border-amber-400 bg-amber-50 shadow-md' : 'border-gray-300 bg-white'}`}>
              <p className="text-[10px] font-bold tracking-wider text-gray-400 uppercase">Merkle Root</p>
              <p className="text-xs font-mono text-gray-600">H(H(AB) + H(CD))</p>
            </div>

            {/* Level 1 connectors */}
            <div className="flex items-start justify-center w-full max-w-sm">
              <div className="flex-1 flex justify-end pr-1"><div className={`w-0.5 h-6 ml-auto mr-[50%] transition-colors duration-200 ${merkleHighlight === 'A' || merkleHighlight === 'B' ? 'bg-amber-400' : 'bg-gray-200'}`} /></div>
              <div className="flex-1 flex justify-start pl-1"><div className={`w-0.5 h-6 mr-auto ml-[50%] transition-colors duration-200 ${merkleHighlight === 'C' || merkleHighlight === 'D' ? 'bg-amber-400' : 'bg-gray-200'}`} /></div>
            </div>

            {/* Level 1 nodes */}
            <div className="flex gap-8 sm:gap-16">
              <div className={`rounded-lg border-2 px-4 py-2 text-center transition-all duration-200
                ${merkleHighlight === 'A' || merkleHighlight === 'B' ? 'border-amber-400 bg-amber-50 shadow-sm' : 'border-gray-200 bg-white'}`}>
                <p className="text-xs font-mono text-gray-600">H(AB)</p>
              </div>
              <div className={`rounded-lg border-2 px-4 py-2 text-center transition-all duration-200
                ${merkleHighlight === 'C' || merkleHighlight === 'D' ? 'border-amber-400 bg-amber-50 shadow-sm' : 'border-gray-200 bg-white'}`}>
                <p className="text-xs font-mono text-gray-600">H(CD)</p>
              </div>
            </div>
            ...
            <div className="flex gap-8 sm:gap-16 w-full max-w-xs justify-center">
              <div className="flex gap-4 sm:gap-8">
                <div className={`w-0.5 h-6 transition-colors duration-200 ${merkleHighlight === 'A' ? 'bg-amber-400' : 'bg-gray-200'}`} />
                <div className={`w-0.5 h-6 transition-colors duration-200 ${merkleHighlight === 'B' ? 'bg-amber-400' : 'bg-gray-200'}`} />
              </div>
              <div className="flex gap-4 sm:gap-8">
                <div className={`w-0.5 h-6 transition-colors duration-200 ${merkleHighlight === 'C' ? 'bg-amber-400' : 'bg-gray-200'}`} />
                <div className={`w-0.5 h-6 transition-colors duration-200 ${merkleHighlight === 'D' ? 'bg-amber-400' : 'bg-gray-200'}`} />
              </div>
            </div>
              ...
              {['A', 'B', 'C', 'D'].map(tx => (
                <div
                  ...
                  className={`rounded-lg border-2 px-3 py-1.5 text-center transition-all duration-200
                    ${merkleHighlight === tx ? 'border-amber-400 bg-amber-50 shadow-sm' : 'border-gray-200 bg-white'}`}
                >
                  <p className="text-xs font-mono text-gray-500">H({tx})</p>
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-2 sm:gap-4">
              {['A', 'B', 'C', 'D'].map(tx => (
                <div key={tx} className="flex justify-center w-[60px] sm:w-[68px]">
                  <div className={`w-0.5 h-4 transition-colors duration-200 ${merkleHighlight === tx ? 'bg-amber-400' : 'bg-gray-200'}`} />
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-2 sm:gap-4">
              {['A', 'B', 'C', 'D'].map(tx => (
                ...
                >
                  <p className="text-xs font-bold text-gray-700">TX {tx}</p>
                </button>
              ))}
            </div>

            {merkleHighlight && (
              <div className="mt-3 rounded-xl bg-amber-50 border border-amber-200 p-3 max-w-sm text-center">
                <p className="text-xs text-amber-800">
                  TX {merkleHighlight} → H({merkleHighlight}) → H({merkleHighlight === 'A' || merkleHighlight === 'B' ? 'AB' : 'CD'}) → <strong>Merkle Root</strong>
                </p>
                <p className="text-[10px] text-amber-600 mt-1">Change this TX and the root changes, breaking the block header hash.</p>
              </div>
            )}
          </div>
        )}
