...
export function P({ children }: { children: React.ReactNode }) {
  return <p className="text-sm sm:text-[15px] leading-relaxed text-gray-600 dark:text-gray-400">{children}</p>
}
...
export function NowBox({ asOf, children }: { asOf: string; children: React.ReactNode }) {
  return (
    <div className="rounded-xl border border-indigo-200 dark:border-indigo-800 bg-indigo-50/70 dark:bg-indigo-950/40 overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2 bg-indigo-100/70 dark:bg-indigo-900/40 border-b border-indigo-200 dark:border-indigo-800">
        <CalendarClock size={13} className="text-indigo-500 dark:text-indigo-400 shrink-0" />
        <span className="text-[11px] font-bold uppercase tracking-widest text-indigo-600 dark:text-indigo-300">
          Snapshot — as of {asOf}
        </span>
      </div>
      <div className="p-4 text-sm leading-relaxed text-indigo-950 dark:text-indigo-100 space-y-2">
        {children}
      </div>
    </div>
  )
      ...
      {steps.map((step, i) => (
        <li key={i} className="flex gap-3">
          <div className="flex flex-col items-center">
            <div className="w-7 h-7 rounded-full bg-amber-100 dark:bg-amber-900/50 border border-amber-300 dark:border-amber-700 flex items-center justify-center text-xs font-bold text-amber-700 dark:text-amber-300 shrink-0">
              {i + 1}
            </div>
            {i < steps.length - 1 && <div className="w-px flex-1 min-h-4 bg-amber-200 dark:bg-amber-800" />}
          </div>
          <div className="pb-4 pt-0.5 min-w-0">
            <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{step.label}</p>
            {step.detail && <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 leading-relaxed">{step.detail}</p>}
          </div>
        </li>
      ))}
    </ol>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {terms.map((t, i) => (
        <div key={i} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3.5">
          <p className="text-sm font-semibold text-amber-700 dark:text-amber-400">{t.term}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-relaxed">{t.definition}</p>
        </div>
      ))}
    </div>
  )
    ...
    <div className="space-y-3">
      {items.map((item, i) => (
        <div key={i} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
          ...
          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{item.explanation}</p>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-colors"
      >
      ...
      {open && (
        <div className="px-4 pb-3.5 pt-1 border-t border-gray-100 dark:border-gray-800">
      ...
      <div className="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900">
        {items.map((item, i) => (
          <div key={i} className="px-4 py-2.5 flex flex-col sm:flex-row sm:items-baseline gap-0.5 sm:gap-3">
            <span className="text-xs font-semibold text-amber-700 dark:text-amber-400 sm:w-40 shrink-0">{item.label}</span>
            <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{item.value}</span>
          </div>
        ))}
      </div>
    </div>
